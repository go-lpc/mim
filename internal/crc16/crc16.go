...
package crc16 // import "github.com/go-lpc/mim/internal/crc16"
...
func (d *digest) update(data []byte) uint16 {
	for _, v := range data {
		d.crc = (d.crc&0xff)<<8 ^ (*d.tab)[(byte(d.crc>>8)^(v&0xff))&0xff]
	}
	return d.crc
}
